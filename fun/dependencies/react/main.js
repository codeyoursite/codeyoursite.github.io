...
let item_two_price = 100;
let item_three_price = 300;
let item_four_price = 500;
let item_five_price = 2000;
let click_amount = 1
let cursors = [];
let wilsons = [];
let nippys = [];
...
function updateWorkerCps() {
    // This check is the most critical part. Ensure the worker exists before posting.
    if (worker) {
        const payload = {
            type: 'updateCps',
...
function startWorker() {
    if (typeof(Worker) !== "undefined" && !worker) {
        ...
        // Add an error listener for debugging purposes.
...
function addBumwacker() {
    if (counter >= item_one_price) {
        ...
        // No need for a setTimeout. Update immediately.
        updateWorkerCps();
...
// Function to add a Nippy (20 CPS)
function addNipps() {
...
// Function to add a Henry (click multiplier)
function addMore() {
